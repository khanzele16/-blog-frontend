import React from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { useNavigate, useParams } from 'react-router-dom'
import PreviewSkeleton from '../../../Components/Skeleton/Posts/Preview/PreviewSkeleton'
import {
	clearPData,
	deletePost,
	deletePPost,
	fetchPost,
	fetchPosts,
} from '../../../Redux/Slices/postsSlice'
import { TabTitleFunction } from '../../../utils/TabTitle.js'
import Banner from '../../Banner/Banner.jsx'
import DeleteAlert from '../../Modals/DeleteAlert/DeleteAlert.jsx'
import Views from '../../Modals/Views/Views'
import PEditor from '../../PEditor/PEditor.jsx'
import PHeader from '../../PHeader/PHeader.jsx'
import Reactions from '../../Reactions/Reactions.jsx'
import './Full.css'

function Full() {
	const { id } = useParams()
	const dispatch = useDispatch()
	const post = useSelector(state => state.posts.one.data)
	const status = useSelector(state => state.posts.one.status)
	const [isViews, setIsViews] = React.useState(false)
	const [deleteAlert, setDeleteAlert] = React.useState(false)
	const viewsButton = React.useRef()
	const navigate = useNavigate()
	React.useEffect(() => {
		dispatch(fetchPost({ id }))
	}, [])
	React.useEffect(() => {
		if (isViews) {
			document.getElementsByTagName('body')[0].style =
				'overflow-y: hidden; margin-right: 15px'
		} else {
			document.getElementsByTagName('body')[0].style =
				'overflow-y: scroll; margin-right: 0px'
		}
	}, [isViews])
	async function handleBtnDelete() {
		await dispatch(deletePost({ id: post?._id }))
		dispatch(deletePPost({ id: post?._id }))
	}
	TabTitleFunction(
		post?.title
			? `${post?.title} — @${post?.author?.username} на .blog`
			: `Пост в блоге ${post?.author?.username} на .blog`
	)
	return (
		<div className='Full'>
			<Banner />
			{status == 'loading' && <PreviewSkeleton />}
			{status == 'loaded' && (
				<div className='Full-post'>
					{deleteAlert && (
						<DeleteAlert
							handleBtnDelete={() => {
								handleBtnDelete({ id: post?._id })
								clearPData()
								fetchPosts({ skip: 0, limit: 10 })
								navigate('/feed')
							}}
							setDeleteAlert={event => setDeleteAlert(event)}
						/>
					)}
					{isViews && (
						<Views
							counter={post?.counter}
							reactions={post?.reactions?.counters}
							viewsButton={viewsButton}
							setIsViews={event => setIsViews(event)}
						/>
					)}
					<PHeader
						post={post}
						setDeleteAlert={event => setDeleteAlert(event)}
					/>
					<PEditor title={post?.title} blocks={post?.blocks} />
					<Reactions post={post} />
					<div className='Full-post-footer'>
						<ul className='Full-post-footer-params'>
							<li id='full-params-li'>
								<svg
									xmlns='http://www.w3.org/2000/svg'
									viewBox='0 0 24 24'
									fill='none'
								>
									<g clipPath='url(#clip0_429_11233)'>
										<path
											d='M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 13.4876 3.36093 14.891 4 16.1272L3 21L7.8728 20C9.10904 20.6391 10.5124 21 12 21Z'
											strokeWidth='2.5'
											strokeLinecap='round'
											strokeLinejoin='round'
										/>
									</g>
									<defs>
										<clipPath id='clip0_429_11233'>
											<rect width='24' height='24' fill='white' />
										</clipPath>
									</defs>
								</svg>
								<p>{post?.comments?.length}</p>
							</li>
							<li id='full-params-li'>
								<svg
									xmlns='http://www.w3.org/2000/svg'
									id='full-params-save'
									viewBox='0 0 32 32'
								>
									<g strokeWidth='1.5' id='Layer_1'>
										<g>
											<path d='m7.4282 30.2998c-.4097 0-.8203-.1064-1.1963-.3193-.7715-.4366-1.2319-1.2266-1.2319-2.1128v-23.8902c0-1.4272 1.1611-2.5884 2.5879-2.5884h16.8242c1.4268.0001 2.5879 1.1612 2.5879 2.5884v23.8901c0 .8862-.4604 1.6763-1.2319 2.1128-.7705.436-1.6851.4243-2.4448-.0313l-7.021-4.2124c-.1865-.1113-.418-.1113-.6045 0l-7.021 4.2124c-.3892.2335-.8184.3507-1.2486.3507zm8.5718-6.6475c.2304 0 .7683.0308 1.3315.3696l7.021 4.2124c.1318.0794.2967.0816.4307.0059.134-.0758.2171-.2185.2168-.3726v-23.8901c0-.3242-.2637-.5884-.5879-.5884h-16.8242c-.3242.0001-.5879.2642-.5879.5884v23.8901c-.0003.154.0828.2967.2168.3726.1339.0758.2988.0735.4307-.0059l7.021-4.2124c.1974-.1187.6743-.3696 1.3315-.3696z' />
										</g>
									</g>
								</svg>
								<p>{post?.savesCount}</p>
							</li>
						</ul>
						<div
							ref={viewsButton}
							onClick={() => setIsViews(true)}
							className='Full-post-footer-eyes'
						>
							<svg
								xmlns='http://www.w3.org/2000/svg'
								viewBox='0 0 24 24'
								fill='none'
							>
								<path
									d='M15.0007 12C15.0007 13.6569 13.6576 15 12.0007 15C10.3439 15 9.00073 13.6569 9.00073 12C9.00073 10.3431 10.3439 9 12.0007 9C13.6576 9 15.0007 10.3431 15.0007 12Z'
									strokeWidth='2'
									strokeLinecap='round'
									strokeLinejoin='round'
								/>
								<path
									d='M12.0012 5C7.52354 5 3.73326 7.94288 2.45898 12C3.73324 16.0571 7.52354 19 12.0012 19C16.4788 19 20.2691 16.0571 21.5434 12C20.2691 7.94291 16.4788 5 12.0012 5Z'
									strokeWidth='2'
									strokeLinecap='round'
									strokeLinejoin='round'
								/>
							</svg>
							<p>{post?.counter?.fullViewsCount}</p>
						</div>
					</div>
				</div>
			)}
		</div>
	)
}

export default Full
